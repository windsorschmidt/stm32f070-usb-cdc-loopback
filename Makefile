##########################################################################################################################
# File automatically-generated by tool: [projectgenerator] version: [3.5.2] date: [Sun Nov 17 00:00:59 PST 2019] 
##########################################################################################################################
...
jlink-gdbserver:
	JLinkGDBServer -device STM32F070CB -if SWD -speed 40000

jlink-gdb: $(BUILD_DIR)/$(TARGET).elf
	arm-none-eabi-gdb -ex "target extended-remote :2331" \
	$(BUILD_DIR)/$(TARGET).elf

jlink-flash: $(BUILD_DIR)/$(TARGET).elf
	arm-none-eabi-gdb -batch-silent -ex "target extended-remote :2331" \
	-ex "mon halt" -ex "mon reset" -ex "load" -ex "mon go" -ex "quit" \
	$(BUILD_DIR)/$(TARGET).elf
